...
type AssetsDataTablePropsLocal =   AssetsDataTableProps & {
  ...
  // ADDED: Filter props passed from parent component
  assetNameKey: string;
...
const AssetRow: React.FC<AssetRowProps & { hiddenColumns: Set<string>; compact: boolean }> = ({
  ...
  const getPhaseData = (phase: string) => {
    ...
    if (workStatusValue) {
        ...
        workStatus = WORK_STATUS[raw] || WORK_STATUS[raw.toLowerCase()] || WORK_STATUS[raw.charAt(0).toLowerCase() + raw.slice(1)];
    ...
    if (approvalStatusValue) {
        ...
        approvalStatus = APPROVAL_STATUS[raw] || APPROVAL_STATUS[raw.toLowerCase()] || APPROVAL_STATUS[raw.charAt(0).toLowerCase() + raw.slice(1)];
  ...
  return (
    <TableRow>
      ...
      {/* PHASE GROUPS (Removed compact check, rely on isHidden) */}
...
const AssetsDataTable: React.FC<AssetsDataTableProps> = ({
  ...
  // ADDED: Destructure the filter props passed from the parent component
  assetNameKey,
  approvalStatuses,
  workStatuses,
}) => {
