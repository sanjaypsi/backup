...
func main() {
	...
	apiRouter := router.Group("/api")
	{
		...
		directoryRepository, err := repository.NewDirectory(gormDB)
		if err != nil {
			log.Fatalln(err)
		}
        ...
        shotPropertyRepository, err := repository.NewShotProperty(gormDB)
        if err != nil {
            log.Fatalln(err)
        }
		...
		apiRouter.GET("/projects/:project/reviews2", reviewInfoDelivery.List2)
		...
		apiRouter.POST("/projects/:project/reviews2", reviewInfoDelivery.Post2)
		...
		/* ____________________________________________________________________________________
			The following API is for pivoting review information by asset. It is designed to support
			efficient retrieval and aggregation of review data, providing a consolidated view
			of the latest submissions and their statuses across different assets.
		____________________________________________________________________________________*/
		apiRouter.GET("/projects/:project/reviews/assets/pivot", reviewInfoDelivery.ListAssetsPivot)

		/* ____________________________________________________________________________________
			The following API is for pivoting review information by shot. It allows clients to query
			review data with various filters and sorting options, enabling them to analyze the
			review statuses of shots in a flexible manner. Clients can specify parameters such as
			ordering keys, direction, pagination, and filters for approval and work statuses to
			customize the data retrieval according to their needs.
		____________________________________________________________________________________*/
		apiRouter.GET("/projects/:project/reviews/shots/pivot", reviewInfoDelivery.ListShotsPivot)
        ...
        shotPropertyUsecase := usecase.NewShotProperty(
            shotPropertyRepository,
            projectInfoRepository,
            readTimeout,
            writeTimeout,
		...
		apiRouter.GET("/projects/:project/shots/generateCsv", generateCsvDelivery.GenerateShotsCsv)

		// PPI Tracker
		ppiTrackerRepository := repository.NewTracker(mongoDB)
		func() {
			setupCtx, setupCancel := context.WithTimeout(context.Background(), 30*time.Second)
			defer setupCancel()
			if err := ppiTrackerRepository.EnsureIndexes(setupCtx); err != nil {
				log.Printf("Warning: Failed to ensure indexes: %v", err)
			}
		}()
		ppiTrackerUsecase := usecase.NewTracker(
			ppiTrackerRepository,
		)
		ppiTrackerDelivery := delivery.NewTracker(ppiTrackerUsecase)
		// Column
		apiRouter.GET("/projects/:project/tracker/columns", ppiTrackerDelivery.GetColumns)
		apiRouter.POST("/projects/:project/tracker/columns", ppiTrackerDelivery.CreateColumn)
		apiRouter.PUT("/projects/:project/tracker/columns/:id", ppiTrackerDelivery.UpdateColumn)
		// Entity
		apiRouter.POST("/projects/:project/tracker/entities/search", ppiTrackerDelivery.SearchEntities)
		apiRouter.POST("/tracker/entities", ppiTrackerDelivery.CreateEntity)
		apiRouter.PUT("/tracker/entities/:id", ppiTrackerDelivery.UpdateEntity)
		// History
		apiRouter.GET("/tracker/entities/:id/history", ppiTrackerDelivery.GetHistory)
		apiRouter.POST("/tracker/entities/:id/rollback", ppiTrackerDelivery.RollbackEntity)
	}
