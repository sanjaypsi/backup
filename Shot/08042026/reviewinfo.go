...
func (u *ReviewInfo) ListShotsPivot(ctx context.Context, p ListShotsPivotParams) (*ListShotsPivotResult, error) {
	...
	shots, total, groupCounts, err := u.repo.ListShotsPivot(
		ctx,
		p.Project,
		p.OrderKey,
		strings.ToLower(dir),
	...
	return &ListShotsPivotResult{
		...
		Sort:     p.OrderKey,
		Dir:      strings.ToLower(dir),
		// Group counts for each top-level group
